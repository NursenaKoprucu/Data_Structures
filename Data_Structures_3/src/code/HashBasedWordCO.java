...
public class HashBasedWordCO {
	...
	String[] ignoreList;
	int minWordLen;
	HashSet<String> ignoreListAsSet;
	...
	public void setIgnoredWords(String[] ignoredWords) {
		...
		ignoreListAsSet = new HashSet<>();
		for (int i = 0; i < ignoredWords.length; i++) {
			ignoreListAsSet.put(ignoredWords[i]);
		}
	}
			...
			if (lineEnders.contains(c)) {
				if (i > 0 && i < n - 1) {
					...
					if (Character.isDigit(text.charAt(i - 1)) && Character.isDigit(text.charAt(i + 1))) {
						...
						sentences.add(text.substring(preI, i).trim().replaceAll("[^A-Za-z0-9 ]+", "")
	...
	public HashSet<String> getUniqueWords(String text) {
		...
		for (String sentence : sentences) {
			for (String word : sentence.split("\\s+")) {
				...
				// the minWordLen
				if (word.length() >= minWordLen && !ignoreListAsSet.contains(word)) {
					uWords.put(word);
				}
		...
		for (String s : sentences) {
			...
			for (int i = 0; i < sWords.length; i++) {
				...
				for (int j = i - windowSize; j <= i + windowSize; j++) {
					if (0 <= j && j < sWords.length && i != j && words.contains(sWords[j])) {
						coMat.get(currWord).increment(sWords[j]);
					}
				}
			}
		}
